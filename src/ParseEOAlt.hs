...
showHead :: (PrintfType t, Data (a b)) => Int -> Node a b -> t
showHead n Node {..} = printf "%s%s %s" (tabs n) (cName node) (show pos)

nothing :: String
nothing = "Nothing"

printTree :: Data b => Node TProgram b -> [Char]
printTree p = printProgram 0 p

-- | for non-leaf nodes
printNonLeaf :: (Data (a b), Foldable t, Functor t) =>
  Int -> Node a b -> t (Int -> [Char]) -> [Char]
printNonLeaf n i l = (showHead n i) <> "\n" <> (foldl (<>) [] (($ n + 1) <$> l))

-- | for leaf nodes
printLeaf :: (Data (a1 b), Show a2) => Int -> Node a1 b -> a2 -> String
printLeaf n i d = showHead n i <>  " " <> (show d) <> "\n"


-- Node printers

printProgram :: Data b => Int -> Node TProgram b -> [Char]
...
printMeta :: Data b => Int -> Node TMeta b -> [Char]
...
-- tTerminal :: Text -> TTerminal () -> Parser (Node TTerminal ())
tTerminal :: Text -> a Load -> ParsecT Void Text Identity (Node a Load)
